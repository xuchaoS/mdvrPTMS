...
class MqClient(object):
    def __init__(self, ip, auth_code):
        ...
        for key in BINDIND_KEY:
            self.channel.queue_bind(queue='shangxcTest', exchange='GPS_EXCHANGE', routing_key='GPS.%s.%s' % (key, auth_code))
        self.channel.queue_bind(queue='shangxcTest', exchange='GPS_EXCHANGE', routing_key='GPS.GpsInfo.%s' % auth_code)
        self.channel.queue_purge('shangxcTest')
        for key in BINDIND_KEY:
            self.channel.queue_declare(queue='shangxcTest_%s' % key, auto_delete=False)
            self.channel.queue_bind(queue='shangxcTest_%s' % key, exchange='GPS_EXCHANGE', routing_key='GPS.%s.%s' % (key, auth_code))
            self.channel.queue_purge('shangxcTest_%s' % key)
        self.channel.queue_declare(queue='shangxcTest_GpsInfo', auto_delete=False)
        self.channel.queue_bind(queue='shangxcTest_GpsInfo', exchange='GPS_EXCHANGE', routing_key='GPS.GpsInfo.%s' % auth_code)
    ...
    def get_message(self):
        ...
        try:
            ...
            logging.info('get message: ' + body.decode('utf8'))
            return method_frame.routing_key.split('.')[1], json.loads(body.decode('utf8'))
        except:
            pass
    ...
    def get_sp_message(self, routing_key):
        try:
            ...
            self.channel.basic_ack(delivery_tag=method_frame.delivery_tag)
            logging.info('get sp message: ' + body.decode('utf8'))
            return json.loads(body.decode('utf8'))
        except:
            pass

    def send_message(self, routing_key, uid, message, exchange='GPS_EXCHANGE'):
        logging.info('exchange: %s, routing_key: %s, send to mq %s' % (exchange, 'GPS.%s.%s' % (routing_key, uid), message))
        self.channel.basic_publish(exchange, 'GPS.%s.%s' % (routing_key, uid), json.dumps(message))
