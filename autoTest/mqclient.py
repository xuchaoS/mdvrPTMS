...
class MqClient(object):
    def __init__(self, ip, auth_code):
        ...
        for key in BINDIND_KEY:
            self.channel.queue_bind(queue='shangxcTest', exchange='MDVR_EXCHANGE', routing_key='MDVR.%s.%s' % (key, auth_code))
        self.channel.queue_bind(queue='shangxcTest', exchange='GPS_EXCHANGE', routing_key='MDVR.GpsInfo.%s' % auth_code)
        self.channel.queue_purge('shangxcTest')
        for key in BINDIND_KEY:
            self.channel.queue_declare(queue='shangxcTest_%s' % key, auto_delete=False)
            self.channel.queue_bind(queue='shangxcTest_%s' % key, exchange='MDVR_EXCHANGE', routing_key='MDVR.%s.%s' % (key, auth_code))
            self.channel.queue_purge('shangxcTest_%s' % key)
        self.channel.queue_declare(queue='shangxcTest_GpsInfo', auto_delete=False)
        self.channel.queue_bind(queue='shangxcTest_GpsInfo', exchange='GPS_EXCHANGE', routing_key='MDVR.GpsInfo.%s' % auth_code)
    ...
    def send_message(self, routing_key, uid, message, exchange='MDVR_EXCHANGE'):
        logging.info('send to mq %s' % message)
